...
public class KafkaLifeCycleController {
     ...
     * @see KafkaMessageListenerContainer.ListenerConsumer#run() 判断{@link AbstractMessageListenerContainer#running}，
     * 当为<code>false</code>时，提交offset，取消订阅
     */
    private static String doStop() {
        String flag = "success";
        logger.info("共有{}个单条消费的context、{}个批量消费的context.", singleConsumerContexts.size(), batchConsumerContexts.size());
        ...
        for (MqKafkaBatchConsumerContext consumer : batchConsumerContexts) {
            try {
                if (!Objects.isNull(consumer)) {
                    consumer.stop();
                }
